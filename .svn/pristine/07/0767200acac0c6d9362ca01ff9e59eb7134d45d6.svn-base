...
<body>
    ...
    let prices = document.getElementsByClassName("shoping__cart__price") // price 객체들 획득
    let totals = document.getElementsByClassName("shoping__cart__total") // total 객체들 획득
	let closes = document.getElementsByClassName("icon_close") // close Button 객체들 획득
	let decBtns = document.getElementsByClassName("dec qtybtn") // 감소버튼 객체들 획득
	let incBtns = document.getElementsByClassName("inc qtybtn") // 증가버튼 객체들 획득
    
	function addEvent(){
    	for(let i = 0; i < incBtns.length; i++){
    		incBtns[i].onclick = function(){
    			
    			let quans = document.getElementsByClassName("quantityVal") // quantity 객체들 획득 
